...
- **ADR-016 — The in-scope test for a cost technique.** The boundary around which of the field's published cost techniques `optio_optimize` implements had twice been drawn on *effort* ("that needs a queue", "that needs a team") — which is a reason for a caller to decline a technique and never a reason for a library to, since absorbing that work is what a library is for. Retires effort and replaces it with three tests: expressible against the normalized types, needs no infrastructure the caller must stand up and secure, measurable by the bench harness. Consequence: tool-schema cost, the largest evidenced win in the literature, stops being out of scope; stop sequences become a request *field* rather than a stage, since a correct one is a fact about the caller's output format.
- **ADR-017 — Batch dispatch is a second surface, not a stage.** Providers sell asynchronous processing at ~50% off with no quality trade, and it cannot be a `Stage`: a stage's contract is that a response returns on the same stack frame, and this one returns tomorrow. Decision: a separate public class, `BatchOptimizer`, which owns an `Optimizer` so the stages run first unchanged and the discounts compose; the caller declares latency tolerance and the library never infers it; the exact cache is shared in both directions; submission failure raises rather than failing open, because there is no degraded path to fall back to. Consequence: ADR-012's rule now covers two entry points, and the batch saving is the project's only *arithmetic* rather than measured figure — the A/B harness cannot express an arm that returns in hours — so it is labelled as such wherever it is printed.
- **ADR-018 — Reasoning budget is a cost lever, and an `ALTERED` one.** `LLMRequest.thinking_budget` was typed, documented, part of the cache key, and in `wire.UNSENT_FIELDS` — so the most expensive tokens in a request (reasoning tokens bill at *completion* rates, 4–5x input on every model in `PRICING`) were the only ones this package could not influence, while nineteen stages aimed at the cheaper half of the bill. Decision: the field gets a real wire path for both vendors, and the stage that uses it is `ALTERED` and off by default. `ALTERED` despite changing not one byte of the prompt, because fidelity is a claim about the *response*: a lower budget is free on easy steps and wrong on hard ones, and hard is why someone chose a reasoning model. The stage only ever lowers a budget the caller set, never raises one and never sets one where there was none. Consequence: correctness must be measured beside cost, never alone — a cost number by itself is evidence for only the half of the trade that flatters the stage; and `chain_of_draft` must not double-count the same avoided reasoning tokens. **Amended after the live run:** −21.9% against two bracketing controls with accuracy unchanged, but **zero of forty control calls came near the ceiling** — so `budget_tokens` is a *target* that shapes how long the model thinks, not only a cap that truncates it, and the stage's original safety argument ("it cannot bind on the observed distribution") was answering a question nobody asked. The flag stays off; the accuracy half of the gate was satisfied in form only, because every arm scored 100%.
- **ADR-019 — A streamed call gets the request-side pipeline, and a replayed cache hit.** Both adapters said "streaming is not optimized" because a pipeline built around one request producing one response "can only buffer a token stream". The first half is true and the conclusion does not follow: most of this package never touches the response, so a streaming caller was getting **zero of nineteen stages** — including `prefix_cache`, the largest lossless saving and Anthropic's only cache mechanism — for a reason that covers only the handful reading a reply. Decision: every `before` hook runs and the transformed request goes on the wire; the `after` hooks run when the stream finishes, from a proxy that forwards each event unchanged and immediately while accumulating alongside; an abandoned stream completes *nothing*, because `exact_cache` storing half an answer would serve a truncation permanently; a cache hit is replayed as a synthesized event sequence built through the SDK's own types. Scope is Anthropic sync and async — OpenAI streaming stays unoptimized and keeps saying so, since shipping "streaming works" while one adapter silently does nothing would repeat the very defect being fixed. Consequence: `complete` must fire exactly once per `prepare` and a stream offers two ways to violate that, so the one-shot guard carries a named test; and a streamed span's timing now covers the whole generation, a different meaning from the non-streaming path's.
- **ADR-020 — Fan-out warm-up is an async dispatch order, not a stage.** N parallel calls over a shared prefix each pay to populate the provider cache because none can see another's write; one first, then N−1 reads, takes the shared prefix from 5×1.25 to 1.25+4×0.1 on Anthropic — **74% off** for a fan-out of five, with no request changed. Decision: a method on `Optimizer` rather than a stage (it answers *when and by whom*, ADR-017's test) but not a second surface either, since responses return on the same stack frame; **async only**, because a synchronous caller looping over five calls already gets warm-up ordering for free and serving a thread-pool caller would mean this package owning a thread pool (ADR-016's second test); opt-in and never inferred, since serializing the head of a fan-out is a latency regression the caller must choose; and warm-up happens only when the leading messages shared by every request clear `MIN_PREFIX_TOKENS`, because below a provider's floor the cache is never populated and the warm-up is a full round trip bought for nothing. Consequence: it claims no saving of its own — the effect lands in `provider_cached_tokens` and `provider_written_tokens`, which are measured rather than estimated; and `prepare`/`complete` now has three callers, which is the argument for that seam being public.
- **ADR-021 — Cache TTL selection, and why its accounting has to land first.** Anthropic sells a 5-minute write at 1.25x and a one-hour write at 2.0x, reads at 0.1x from either; this package has only ever asked for five minutes, which is wrong for any agent step slower than that — the next step pays a fresh write on a prefix it just wrote. Break-even is one further use inside the hour (`2.0 + 0.1m < 1.25(m+1)` holds for `m >= 1`). Probed before writing: `ttl: "1h"` needs **no beta header**, and the provider already reports writes split into `ephemeral_1h_input_tokens` / `ephemeral_5m_input_tokens`. Decision: **the accounting ships before the lever** — `_cost` prices every write at 1.25x today, so asking for 1h would under-bill the most expensive band by 37.5% in the direction that inflates this package's headline, which is the identical asymmetry that already turned a published 53.7% into 50.1%; TTL selection lives inside `PrefixCacheStage` rather than a second stage that would rewrite its decision; one hour is requested only after expiry has been **observed** (the same prefix seen again after a gap exceeding five minutes), never predicted; a prefix is identified by hash, never text (§10), in an LRU bounded per §11. Consequence: it ships off by default despite being `IDENTICAL`, because it is the first lever here that can *raise* a bill and rule 1 forbids that — and the live run that decides the default needs real five-minute waits, 16 minutes of wall clock. **Amended after that run:** every mechanical claim held — four rounds 330s apart, the control re-writing all four (which is what makes the result meaningful), the treated arm writing at 5m in round 1 exactly as an observe-first design must, then 1h, then two reads, for **−30.9%** — and the cumulative curve settled the default the other way from the headline: after round 2 the treated arm is **29.9% more expensive**, because the upgrade write costs 2.0x and pays back only on the round after. A prefix that expires once and is never used again loses ~30% permanently, which is a two-turn conversation resumed after a break. Observed expiry is sound *backward* evidence; the upgrade needs a forward fact the run measured nothing about, and rule 1 does not say "reduce cost in expectation". Recorded but not implemented: requiring **two** observed expiries is a materially better bet with its own break-even, so it is an open question rather than code.
- **ADR-022 — An image is content, and the cache key was the urgent half.** Investigating spec item 5's *counting* gap turned up a wrong-answer bug that outranks it: `request_key` keys `[role, content, name]`, an image block never reaches `Message.content`, and `UNKEYED_FIELDS` excludes `extra` as "provider transport details, not semantics" — a classification that does not hold when the image **is** the semantics. Verified: two requests with identical text and different images produce the identical digest `277f0c84…`, and `exact_cache` is **on by default** at `temperature == 0`, precisely the setting deterministic vision work uses. Same defect class as the `stop` bug documented in that same dict. The counting gap is narrower than it first looks and the correction matters: `actual_input` takes the **provider's** number on a live call, so `reduction_ratio` is *not* inflated — the damage is that `count_request` returns **8 tokens** for a request billing ~1,535, which makes `fits_in_window` unsafe (under-counting a limit decision is the provider-side rejection that function exists to prevent) and understates savings on the short-circuit path where no provider number exists. Measured, not modelled, via Anthropic's free exact `count_tokens`: `(w*h)/750` is accurate to ±4% from 512x512 through 1200x958 and then breaks — a max edge of 1568 plus an aspect-dependent area cap hold everything near **1,600 tokens**, so the bare formula overstates 1568x1568 by **2.15x** and 400x3000 by 3.6x, while small images cost *more* than it says (64x64: 13 against 5). Decision: the key fix ships first and alone; images are counted by a clamped, floored area estimator with dimensions from a dependency-free header parse (§4.4 — Pillow is a native wheel for four integers); missing dimensions yield a documented constant marked inexact, never `0`, because absence is not zero; `count_tokens` stays out of the counter, whose protocol demands determinism and microseconds, and lives in the measurement scripts; and the `detail: "low"` / downscaling **lever is deferred** with its gate named, because ADR-015 wants a vision *accuracy* probe with a hard set the control sometimes gets wrong — the control ADR-018's run was criticized for lacking. Consequence: `exact_cache`'s hit rate on vision requests drops to near zero, which is a saving this package should never have claimed, since those hits were wrong answers.
- **ADR-023 — Cascade routing calls cheap, verifies, and escalates.** `route_models` decides cheap-vs-expensive *before* the call, from prompt length, and its own docstring records the guess failing: *"What is 17 times 24, minus 89?"* routes on every rule and the cheap model answers **329** against 319. That is the proxy shape ADR-018 already rejected for reasoning budgets, with the same silent failure — a fluent, well-formed, wrong answer no cost dashboard and no token-identity check can catch. Cascade removes the guess: call cheap, verify, escalate on failure, so the worst case becomes slow-and-right instead of wrong-and-cheap. Decision: it is a **provider-call wrapper, not a stage**, because a stage's contract (`before → call → after`) is exactly one provider call and this needs two — the same wall ADR-017 hit with batch dispatch, and the same conclusion, so the stage contract stays single-call and the technique that breaks it lives outside `stages/`. The verifier is **caller-supplied with a structural default**: a model judge grading every routed request would spend the escalation it exists to avoid, so `default_verifier` catches only what a cheap deterministic check *can* — empty, truncated, malformed JSON, a proposed tool call naming an unknown tool or missing a required argument — and its docstring says plainly that it does not catch the 17×24 case. A **rejected cheap answer is never cached**, and structurally rather than by special case: the cheap attempt runs inside the wrapper against the raw provider call, so no cache `after` hook ever sees it — the alternative, keying attempts by model, would have reopened the cache key ADR-022 had just finished getting right. Live gate: 12 graded probes, gpt-4o vs gpt-4o-mini, $0.0013; the motivating failure reproduced exactly, and with an oracle verifier cascade scores 12/12 at ~14% of all-expensive cost against static routing's 11/12 at 6%. Consequence: **the verifier is the fidelity claim, not the cascade** — with the built-in default, "329" passes and cascade scores exactly what static routing scores, so the saving must always name its verifier; and cascade's own economics needed their own accounting, since wrapping the provider call means an escalated request's wasted cheap attempt was spent at the provider and counted nowhere until `CascadeStats.cost_summary` made it visible. Ships **off by default**: the live run clears the mechanism, not the judgment ADR-015 reserves for whoever supplies `cheap_model`.
- **ADR-040 — A field with a default is a field every old call site keeps compiling around.** A static review of `main...HEAD` — 127 files, no tests run — reported eight findings; **seven were real**, against a suite green at 2,110 tests and `mypy --strict` clean. Four are one defect: ADR-021 added `cache_write_tokens` and `cache_write_1h_tokens` to `LLMResponse`, every site that *reads* usage was updated, and every site that *copies, zeroes or re-prices* a response was not — both fields carry a `0` default, so all of them kept compiling and kept type-checking. `served_from_cache` zeroed three fields and copied both write bands forward, so every `exact_cache` and `semantic_cache` hit re-billed the original call's premium tokens; `_actual_cost` priced writes at base rate, so `SpendGuard` undercounted against a live `--cap`; the streaming accumulator never read `cache_creation.ephemeral_1h_input_tokens`, so every streamed reply priced 2x tokens at 1.25x. Three are independent: `count_tools` still applied `gpt-4o-mini`'s 0.65 where ADR-036 measured Anthropic at **1.29** — different in direction, since Anthropic re-renders schemas and bills *more* than the raw JSON — a ~2x undercount feeding `PrefixCacheStage`'s floor, so a tool-heavy prefix genuinely over 4,096 measured ~2,000 and was declined (half of the failure ADR-036 was written to fix, still live in the other half); `_warm_counter` passed `""`, which `encoding_for_model` rejects, so only `o200k_base` was warmed and every `cl100k_base` model still paid the 395 ms load inside a 100 ms deadline on request one; and `response_from_anthropic_message` set no `extra`, so a `tool_use` reply reached `default_verifier` as empty content with no proposal and **every** tool-using cascade step escalated — cheap model *and* expensive model, strictly worse than not routing. Two further defects surfaced while fixing those: `cache_ttl_selection` identified a prefix by `messages[:len-2]`, which grows every turn, so in any appending conversation the digest never matched and the one-hour TTL could not be emitted for the agent loop it exists for (identity now covers the system block alone — the part that does not grow); and `last_decline_reason` survived onto a successfully marked request, answering ADR-027's "why did this not cache?" about the wrong one. The eighth finding — that `ReasoningBudgetStage` ratchets toward `MIN_THINKING_BUDGET` — is **wrong**, and is rejected with tests rather than an argument: `REASONING_CEILING_MULTIPLIER` is 2.0, so a budget falls only while the model uses under half of it, and a 60-turn simulation settles at 6,000 / 32,000 / 32,000 / 25,600 across four model behaviours; a mutation setting the multiplier to 1.0 fails those tests, because that is what would actually cause the reported failure. Consequence: the rule is that adding a defaulted field to a shared dataclass silently changes every site that constructs, copies or reduces it, and `mypy --strict` cannot see it — that is what the default is *for*. The countermeasure is a test enumerating the fields, so the next band added fails on the day it appears.

- **ADR-039 — A measurement that costs money to re-check gets checked once.** Every live figure in this repository was obtained by running a script, reading the terminal, copying the number into an ADR by hand, and discarding the response. Forty ADRs and **not one recorded exchange anywhere in the tree** — no fixtures, no cassettes, no stored results. The numbers are not therefore wrong; the problem is narrower and worse: **re-checking one costs money**, so it is checked once and trusted thereafter. That is the exact shape of the longest-lived defect in the changelog — `prefix_cache` reporting a saving for months against a 3,614-token prompt below Anthropic's cacheable minimum, where the live API returned `reads 0 writes 0` — because noticing required a paid call. Decision: `RecordingProvider` appends every (request, response) pair to JSONL as it happens; `ReplayProvider` serves them back with no key, no network, no spend (`--record` / `--replay` on the bench CLI). What a replay proves is narrow and stated as such: **the library still builds the request the provider was measured on** — not that the provider still answers that way, which is why the recording carries its UTC date and `label` prints it. The key is deliberately *not* `cache.request_key`: that omits `max_tokens` and ignores `cacheable` markers, both right for serving a cached answer and wrong for evidence about one exchange — a key blind to `cacheable` would replay the old cache numbers after `prefix_cache` stopped emitting them and report the saving intact, the original defect with a receipt attached. `blake2b` not `hash()`, since string hashing is salted per process and a `hash()`-derived digest would agree with itself all session and disagree with yesterday's recording; a subprocess test with a different `PYTHONHASHSEED` pins it because nothing in one process can. A miss raises rather than answering — a replay that fabricated would credit this library with a saving no provider measured. Repeats replay in recorded order, and `reset()` is a deliberate no-op: two identical requests are how caching is observed at all (the first writes, the second reads, and the responses differ), so rewinding at an A/B arm boundary would serve the baseline's answers to the optimized arm and collapse the measured saving to zero, silently. Consequence: a recorded benchmark replays to identical numbers through the real `compare()`, which also tests something nothing else covers — any hidden per-run variation in request assembly surfaces as a replay miss and nowhere else. `is_live` stays `False`: the tokens are the live run's, the clock is a file read.

- **ADR-038 — The first request paid the tokenizer's startup out of its latency budget.** `tiktoken` loads its BPE vocabulary lazily on first use — **395 ms** against a 100 ms `latency_budget_ms` — so whichever stage counted first paid it out of the deadline and everything after that stage was skipped. Measured on a default config: **five of nine stages never ran**, including `prefix_cache`, the largest lossless saving here and Anthropic's only cache mechanism. Invisible to every benchmark, because request two runs all nine and the aggregate looks fine; worst in exactly the deployments that make one call per process. Decision: warm the counter in `Pipeline.__init__`, where the cost belongs — a one-time cost of *having* an optimizer, not of using one; warming failures are swallowed, since this changes *when* a cost is paid and must never become a new way to fail; construction is allowed to be slow and says so. Consequence: the invariant gets a name and a test — **the first request must not be optimized less than the second** — stated against a deliberately slow counter so it holds for any lazy initializer rather than for `tiktoken` specifically. Found while adding ADR-037, which relocated the cost far enough to fail a test; the bug itself predates it and nothing had ever noticed.
- **ADR-037 — The window binds the prompt, and a separate cap binds the reply.** `context_limit` had been accepted, validated and read by nothing since the first release, and so had `fits_in_window`. Before giving them a consumer, the failure they would prevent was measured rather than assumed — and the obvious design turned out to rest on a false premise. On `claude-haiku-4-5`: a prompt over the window is a hard 400 (`prompt is too long: 217570 tokens > 200000 maximum`), `max_tokens` over the model's **output cap** is a hard 400 naming the cap, and **`prompt + max_tokens` over the window is simply accepted** — 158,965 + 21,000 against a 200,000 window generated normally. A guard on that sum, which is what was about to be built, would have prevented a failure that does not happen. The probe also found a limit this package had never modelled: a per-model output maximum ranging **32,000 to 128,000**, unrelated to the window, that `AdaptiveMaxTokensStage` can exceed on a request that carried no `max_tokens` at all. Decision: two tables recording only provider-stated numbers (seven models whose window is known merely to exceed the probe are **absent**, not guessed at 1M); `WindowPressureStage` observes and reports, never enforces; the guard clamps `max_tokens` to the **output cap**, the limit that is actually enforced; an unknown model gets no opinion. Consequence: the diagnostic runs **last**, opposite to `detect_unstable_prefix` — it reports on the request about to be sent, and placed first it cost **152 ms against the 100 ms budget** on a 2.7 MB conversation, starving the trim that would have fixed that very request. Learning the table cost **$7.60**: the first probe assumed a long prompt guaranteed rejection, and seven models accepted and billed it while the script printed "Spend: nothing".
- **ADR-036 — Tool-schema calibration is per-vendor, and ours was OpenAI's.** `ANNOTATION_STRIP_CALIBRATION = 0.37` was fitted against `gpt-4o-mini` and applied to every vendor. Measured free and exactly via Anthropic's `count_tokens` across five tool counts and three models: **1.29 at every size**, so `minify_tools` understated its own saving by **71.4%** on Anthropic. The vendors differ in *direction*, not just magnitude — OpenAI bills less than the raw JSON, Anthropic more, because it re-renders the schema — so no single constant can be right for both. Decision: a per-vendor lookup keyed on model prefix; an unrecognised model keeps **0.37**, the lowest measured, so an unknown vendor is under-claimed rather than flattered. Fourth instance of one vendor's answer applied globally, after ADR-025, ADR-032 and ADR-033: **anything measured against one vendor is that vendor's until measured against the next.**
- **ADR-035 — A provider serves the request it is given.** The bench providers ignored `request.model` and always called the configured one, so `route_models` and `cascade_routing` were structurally unmeasurable through the benchmark built to validate them. A test had *frozen* this, its own comment noting it made those stages "a no-op under `bench --live`". Decision: the provider honours `request.model`, prices the estimate against it, and records the actual against whatever the API says it *served*.
- **ADR-034 — Cascade pays by cost-weighted escalation, not by count.** Cascade's headline reported a 50% escalation rate against a 66.7% break-even while **losing 25.3%**: the rate counts requests and the bill weights them, and the escalating four were **92% of the spend**. That correlation is structural — long, tool-carrying, schema-demanding requests both fail verifiers and cost more — so a count-weighted rate flatters cascade on any real workload. Decision: report `cost_weighted_escalation_rate` beside a computed `break_even_escalation_rate`.
- **ADR-033 — Truncation is a question, not a string comparison.** Every truncation check compared `finish_reason` against `"length"`, which is OpenAI's vocabulary; Anthropic sends `"max_tokens"`. So the checks were **dead code on Anthropic in two places**, and the serious one was not cascade: `ExactCacheStage` refuses to serve a truncated entry *because* `request_key` omits `max_tokens`, so with the guard inert a `max_tokens=16` call could poison the cache for every later caller — in a lossless, on-by-default stage. Decision: `LLMResponse.was_truncated` against a `TRUNCATION_REASONS` set; no stage compares a provider's string to a literal.
- **ADR-032 — `cap_tool_results` is blind to the shape Anthropic callers send.** The stage read tool results from `Message.content`, which is where the OpenAI adapter puts them; the Anthropic adapter sends `tool_result` content blocks, so the stage was a **complete no-op for every Anthropic adapter user**. Decision: `wire` grows block-aware helpers and the stage handles both shapes. An over-eager cap was invisible in testing because it makes the result *longer*, so `saved` goes negative, the stage declines, and the request comes back looking untouched.
- **ADR-031 — A published price change is data, not a prediction.** The vendor's page lists Sonnet 5 twice — 2/10 through Aug 31 2026 and 3/15 from Sep 1 — so whichever single number were written would be wrong by 50% on one side of the boundary, larger than most savings this package reports. Decision: `_SCHEDULED_PRICING` records dated commitments from the vendor's own page, resolved per call rather than at import so a long-running process does not keep serving the stale rate. Recording a dated vendor commitment is not the prediction this project has a rule against.
- **ADR-030 — A breakpoint nobody reads is pure cost.** A live Sonnet 4.5 run wrote 20,333 prompt tokens at the 1.25x premium, read back **zero**, and came in **23.5% more expensive than doing nothing** — on a prompt whose head carried a timestamp, which `detect_unstable_prefix` had already diagnosed. Decision: the diagnostic publishes `PREFIX_IS_UNSTABLE` and `PrefixCacheStage` reads it; the stage also learns from the provider's own outcome and stops marking after `MAX_UNREWARDED_WRITES` unrewarded writes. Absence of the observation means "not established", never "stable". Established the pattern of one stage publishing a named observation another acts on; −23.5% became −6.0%.
- **ADR-029 — A price may not be inferred across model generations.** Prefix matching priced `claude-opus-4-5` off `claude-opus-4-1`'s row, and Anthropic cut Opus list pricing at 4.5 — so the cost lane reported **$30 on a $10 bill**. Decision: a prefix match counts only when the suffix denotes the *same* model (a release date or revision tag), never a newer generation; an unmatched model returns `None`, never a neighbour's row.
- **ADR-028 — A cost delta is only a measurement when the arms differ.** Five workloads were given percentages for runs where the optimizer provably did nothing: identical requests, identical call counts, and a delta that was pure provider-side sampling noise. Decision: each arm fingerprints what it actually sent, and `ABResult.cost_is_attributable` is false when both digests and both call counts match — the report then prints `NOT ATTRIBUTABLE` rather than a number.
- **ADR-027 — The cacheable prefix floor is per-model.** A single 1,024-token floor was wrong for most of the table: Opus 5 is 512, Haiku 4.5 and Opus 4.6/4.5 are **4,096**. Marking a breakpoint below a model's floor buys a write premium the provider will never let you read back. Decision: per-model floors with longest-prefix matching, and an unknown model gets the **highest** floor — when the table cannot answer, fail toward the number that cannot overstate.
- **ADR-026 — Trimming must price the output it buys.** `trim_history` reported only the input tokens it removed, which is the cheaper half of what it changes.
- **ADR-025 — The neutral tool shape needs an Anthropic translation.** Tool roles and schema shapes were written against OpenAI's wire format and applied to Anthropic. The first of the four instances that became the vendor rule stated in ADR-036.
- **ADR-024 — A stage may not book a saving it cannot attribute.** The first end-to-end live run of `scripts/real_agent_run.py` (four scenarios, both arms, `gpt-4o-mini`, reproduced twice) found the package making two scenarios **more expensive** while reporting a saving: `parallel` −3.0% against a claimed 10.0%, `empty_result` −4.3% against a claimed 13.2%. Isolation returned the input to *exactly* the control arm's figure (661→635, 523→497), so one stage owned the whole regression — and on the two large scenarios it was marginally worse than not running, so it paid for itself in none of the four. Root cause: `StructuredOutputStage`'s docstring says "only acts when a schema is already present" while its guard read `response_format is None and not request.tools`, so it also fired on any tool-using request carrying **no schema at all** — which is every call of every agent scenario in this repo. It appended a JSON-preamble instruction to replies that are *tool calls*, and the measured output confirms nothing was suppressed: 132→**137**, 94→**95**, 28→**28**, 165→148. The accounting compounded it by booking `max(0, 40 - instruction_cost)` output tokens from a preamble nothing ever verified — against `savings.py`'s own opening rule, "only count what was avoided, never what was hoped for" — while the tokens it *added* silently inflated the baseline, reporting 635 where the measured truth was 525. Decision: `structured_output` ships **off by default** (rule 1 forbids the increase, and its benefit has never been measured on a request that actually carries a schema); the guard matches the documented intent, a schema and not tools; no stage reports a saving it cannot attribute, so the hypothesised output figure is gone and the effect, if real, must appear in provider-measured `actual_output_tokens` as ADR-020 established for fan-out; and a stage that **adds** tokens reports that as a *negative* saving, so `baseline = actual + saved` yields the true baseline — 523 + (−26) = 497, exactly what the control billed. `ConcisionStage` carries the identical pattern and gets the identical accounting fix. Consequence: a report can now show a negative saving, which is the point — a stage that costs more than it saves was structurally invisible for the package's entire history, and rounding a loss up to zero is how a 4.3% increase came to be published as a 13.2% gain. Verified live after the fix: the regression is gone (0.0% and −0.8%, the latter output-sampling noise on byte-identical input) and the large wins are intact at +47.9% and +62.2%.
