...
the user's existing tracer provider. That placement is what makes ``agentmeter``
...
every signal agentmeter emits is run-scoped (``docs/signals.md``).
...
if TYPE_CHECKING:
    ...
    from agentmeter.lanes.base import Lane, RunLike
...
def is_genai_span(span: ReadableSpan) -> bool:
    ...
    * Spans carrying **only** attributes agentmeter itself emitted. Writing
...
class AgentMeterSpanTap(SpanProcessor):
    ...
    def on_run_end(self, run: RunLike, span: Span | None = None) -> None:
        ...
        Called by :class:`~agentmeter.runtime.run_context.RunContext` at run end,
