...
SC-4, R-TECH-4). ``agentmeter`` runs in-process, on the critical path of every
...
3. **Setup errors are exempt.** :class:`~agentmeter.errors.AgentMeterConfigError`
...
_NEVER_ABSORB: Final[tuple[type[BaseException], ...]] = (AgentMeterConfigError,)

#: Fail-open activations, keyed by ``(component, exception type name)``. Feeds
#: the ``agentmeter.internal.lane_errors`` self-metric (Section 12) and is the
...
def _record(component: str, exc: BaseException) -> None:
    ...
    try:
        ...
        if component not in _logged_components:
            ...
            _log.warning(
                "agentmeter: %s failed with %s; signal dropped, agent unaffected. "
...
def guard(
    ...
    **kwargs: Any,
    ...
    Raises:
        AgentMeterConfigError: Setup errors are never absorbed.
...
def activation_count(component: str | None = None) -> int:
    ...
    ``agentmeter.internal.lane_errors`` self-metric (Section 12). A rising count
