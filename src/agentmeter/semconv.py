...
the dependency graph (Section 3.1); nothing in ``agentmeter`` may be imported here.
"""
...
#: Namespace root for agentmeter's *self*-observability (Section 12). Deliberately
#: separate from ``gen_ai.*`` so our own health metrics can never be mistaken for
#: -- or gated on by -- a consumer's policy.
INTERNAL_NAMESPACE: Final = "agentmeter.internal"
...
INTERNAL_SIGNALS_EMITTED: Final = "agentmeter.internal.signals_emitted"
INTERNAL_LANE_ERRORS: Final = "agentmeter.internal.lane_errors"
INTERNAL_OVERHEAD_SECONDS: Final = "agentmeter.internal.overhead"
INTERNAL_SAMPLING_RATE: Final = "agentmeter.internal.sampling_rate"


#: Every signal agentmeter emits, as a set. The contract test asserts this set
