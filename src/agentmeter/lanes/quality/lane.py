...
from agentmeter.lanes.quality import heuristic
from agentmeter.lanes.quality.judge import JudgeRequest, JudgeRunner, JudgeScores
from agentmeter.lanes.quality.sampling import Tier, decide
...
class QualityLane(Lane):
    ...
    def __init__(self, config: Config, judge: Judge | None = None) -> None:
        ...
        Args:
            ...
            judge: The user's evaluator. ``None`` means heuristic-only --
                agentmeter ships no default judge, because a default would
        ...
        if config.quality_lane and judge is None:
            ...
            _log.warning(
                "agentmeter: quality lane is enabled but no judge was supplied; "
    ...
    def _judge_scores(
        ...
        # Content is the caller's to provide. agentmeter passes no trace text of
