"""The in-process ledger backend -- the default (ADR-005, ADR-050).

This is the implementation that shipped as ``CostLedger`` through 0.3.0, moved
behind :class:`~optio.lanes.cost.ledger_store.LedgerStore` **unchanged**. Moving
it verbatim is deliberate: the existing cost-lane tests then act as the
regression net for the extraction, and any behavioural difference is a bug in
the move rather than a design choice nobody wrote down.

It holds the invariant the product's core number depends on (**R-TECH-1**):
...
**Single-process by construction.** A lock says nothing to another process, so
runs sharded across processes need the Redis backend; each process here would
meter a fraction of the truth while believing it had the whole.
"""
...
class InMemoryLedgerStore:
    """Per-run reserve/reconcile accounting in a process-local dictionary.
    ...
    :class:`~optio.lanes.cost.ledger_store.LedgerStore` rather than inheriting
    it, matching how ``InMemoryStateStore`` related to its own contract.
    """
    ...
    def close_run(self, run_id: str) -> LedgerSnapshot:
        ...
        with self._lock:
            ...
            if leaked:
                _log.warning(
                    "optio: run %s ended with %d unreconciled reservation(s); "
    ...
    def __repr__(self) -> str:
        ...
        return f"<InMemoryLedgerStore runs={self.run_count()}>"
