...
* :class:`OptioInternalError` and its subclasses are raised *inside*
...
* :class:`OptioConfigError` is the opposite: it is raised at ``instrument()``
...
class OptioError(Exception):
    """Base class for every exception this library defines."""


class OptioConfigError(OptioError):
...
class UnsupportedFrameworkError(OptioConfigError):
    """The target object matches no known adapter."""


class OptioInternalError(OptioError):
...
class LedgerInvariantError(OptioInternalError):
...
class StateStoreError(OptioInternalError):
    """The state store was unreachable or returned an unusable value."""


class SignalWriteError(OptioInternalError):
