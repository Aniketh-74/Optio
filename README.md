...
The Redis row is one round trip by construction — a step is a single Lua script that appends,
evicts, and reduces server-side. It returns four numbers regardless of window size, which is what
keeps the flatness guarantee true across a network.
